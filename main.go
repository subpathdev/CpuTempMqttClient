...
func init() {
	...
	flag.StringVar(&user, "user", "", "is the user, which should used by the mqtt connection")
	flag.StringVar(&password, "password", "", "is the password of the MQTT User")
	flag.BoolVar(&simulate, "simulate", false, "if you use this flag the input data will be siulate by random numbers and no senser will be requested")
}
			...
			for _, element := range str {
				if core0 == true {
					if strings.Contains(element, "temp2_input") {
						...
						if err != nil {
							log.Printf("Error by converting string to int. String is: %v\nError is: %v", val[1], err)
							continue
						}
						message = i
						core0 = false
